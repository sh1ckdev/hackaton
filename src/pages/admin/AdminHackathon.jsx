...
const EMPTY_ITEM = { title: '', description: '', date: '', date_to: '', show_countdown: false };

const AdminHackathon = () => {
  useDocumentTitle('Хакатон — Админ');
  ...
  const saveItem = async (item) => {
    try {
      const payload = { type: item.type || 'other', title: item.title, description: item.description, date: moscowInputToUtc(item.date) || item.date, date_to: item.date_to ? moscowInputToUtc(item.date_to) : null, show_countdown: !!item.show_countdown };
      if (item.id) {
        const res = await api.put(`/admin/settings/timeline/${item.id}`, payload);
        setTimeline(prev => prev.map(t => t.id === item.id ? res.data.timeline_item : t));
      } else {
        const res = await api.post('/admin/settings/timeline', payload);
        setTimeline(prev => [...prev, res.data.timeline_item].sort((a, b) => new Date(a.date_to || a.date) - new Date(b.date_to || b.date)));
      }
      setEditing(null);
    ...
    <div className="p-6">
      <div className="admin-settings-content">
        <div className="admin-settings-section-header">
          ...
          <button onClick={() => setEditing(EMPTY_ITEM)} className="admin-btn-primary">+ Добавить событие</button>
        </div>
            ...
            <div key={item.id || idx} className="admin-settings-item">
              <div className="admin-settings-item-content">
                ...
                <div className="admin-settings-item-meta">
                  {item.date_to
                    ? <span>От {item.date ? new Date(item.date).toLocaleString('ru-RU') : '—'} до {new Date(item.date_to).toLocaleString('ru-RU')}</span>
              ...
              <div className="admin-settings-item-actions">
                <button onClick={() => setEditing(item)} className="admin-btn-secondary">Редактировать</button>
            ...
            <div className="admin-modal-content">
              ...
              <form onSubmit={e => { e.preventDefault(); saveItem(editing); }}>
                ...
                <div className="admin-form-group">
                  ...
                  <input type="datetime-local" value={utcToMoscowForInput(editing.date)} onChange={e => setEditing({ ...editing, date: e.target.value })} className="admin-input" />
                  <small style={{ color: 'var(--text-muted)' }}>Московское время</small>
                </div>
                <div className="admin-form-group">
                  <label>Дата окончания (до)</label>
                  <input type="datetime-local" value={utcToMoscowForInput(editing.date_to)} onChange={e => setEditing({ ...editing, date_to: e.target.value })} className="admin-input" />
                  <small style={{ color: 'var(--text-muted)' }}>Московское время. Если указано — покажет период</small>
                </div>
                <div className="admin-form-group">
