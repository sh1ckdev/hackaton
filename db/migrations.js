...
async function applySecurityAuditMigration() {
  try {
    await pool.query(`
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS idx_request_audit_created_at ON request_audit(created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_request_audit_ip_created_at ON request_audit(ip_address, created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_request_audit_user_created_at ON request_audit(user_id, created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_auth_audit_created_at ON auth_audit(created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_auth_audit_ip_created_at ON auth_audit(ip_address, created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_auth_audit_user_created_at ON auth_audit(user_id, created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_security_events_created_at ON security_events(created_at DESC)');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_security_events_ip_created_at ON security_events(ip_address, created_at DESC)');

    logInfo('Миграция security audit: таблицы и индексы готовы');
  } catch (error) {
    logWarn('Предупреждение при миграции security audit', { error: error.message });
  }
}
